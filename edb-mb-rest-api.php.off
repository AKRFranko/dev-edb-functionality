...
class MB_Rest_API {
  ...
  public function get_post_meta_rest_api( $object ) {
    ...
    // $output['edb_wireframe'] =rwmb_meta( 'edb_wireframe', null, $object['id'] ); 
    // var_dump($output);
    return $output;
  }
   ...
   * @return array
   */
  public function update_post_meta_rest_api( $post_data, $object ) {
    $output = array();
  ...
  /**
   * Set term meta for the rest API.
   *
   * @param array $object Term object
   *
   * @return array
   */
  public function update_term_meta( $json, $object ) {
    // $output = array();
    $output = array();
    $post_data = json_decode( $json, true );
